...
def count_parameters(model, input_size=[(1, 3, 224, 224)], cpu=True):
    ...
    for size in input_size:
        dummy_inputs.append(torch.randn(size).to(device))
    macs, params = profile(model, dummy_inputs, verbose=False)
    #-------------------------------------------------------------------------------#
...
def export2onnx(model, input_size=[(1, 3, 224, 224)], model_name="model.onnx", opt_version=20):
    device = torch.device('cpu')
    model.to(device)
    model.eval()
    ...
    if len(input_size) == 1:
        # Single input case
        dummy_input = torch.randn(input_size[0]).to(device)
        input_names = ["input"]
    else:
        # Multiple inputs case
        dummy_input = []
        for size in input_size:
            dummy_input.append(torch.randn(size).to(device))
        dummy_input = tuple(dummy_input)
        input_names = [f"input{i}" for i in range(len(input_size))]
